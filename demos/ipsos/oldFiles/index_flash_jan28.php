...
function is_ip($str){
	...
	if ($form_game == "npn"){
		...
		if ($err == ""){
			...
			$run = runSql("select `plays` from `survey` where `uid` = '{$vars[0]}' and `sid` = '{$vars[2]}'");
			$row = mysql_fetch_assoc($run);
			...
			if ((is_ip($vars[0]) && $row['plays'] <= 17) || (!is_ip($vars[0]) && $row['plays'] <= 3)){
				...
				if ($vars[1] != ""){
					$rec = 0;
					$run = runSql("select `id` from `played` where uid = '{$vars[0]}' and `id` = '{$vars[1]}'");
				...
				while ($row = mysql_fetch_assoc($run)){
					...
					$query["pimg$p"] = "http://www.ipsospollpredictor.com{$row['img']}";

					$run2 = runSql("SELECT `chances` FROM `prizechances` WHERE pid = {$row['id']} and `uid` = '{$vars[0]}'");
				...
				if (!$vars[1]){
					...
					if (mysql_num_rows($run)){ // increment plays for this user/survey
						$run = runSql("update `survey` set plays = plays + 1 where `uid` = '{$vars[0]}' and `sid` = '{$vars[2]}';");
