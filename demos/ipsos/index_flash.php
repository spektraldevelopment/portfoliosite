...
if ($_GET['r'] == "") $_GET['r'] = "iegqttriegtetb1";
	...
	if ($form_game == "npn"){
		...
		if ($err == ""){
			...
			if (1){
				...
				$run = runSql("SELECT * FROM `prize` ORDER BY expire DESC LIMIT 4");
				$p = 1;
				$ptags = array("<p>", "</p>");
				while ($row = mysql_fetch_assoc($run)){
					...
					$query["pimg$p"] = "http://staging.lushconcepts.com/demos/pollpredictor/{$row['img']}";
